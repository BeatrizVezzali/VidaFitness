...
export default class ItemDatabase {
        ...
        return new Promise((resolve) => {
            ...
            SQLite.echoTest().then(() => {
                ...
                SQLite.openDatabase(database_name, database_version, database_displayname, database_size).then(DB => {
                    ...
                    db.executeSql('SELECT 1 FROM cadastroItem LIMIT 1').then(() => {
                        ...
                        db.transaction((tx) => {
                            ...
                            tx.executeSql('CREATE TABLE IF NOT EXISTS cadastroItem (id INTEGER PRIMARY KEY AUTOINCREMENT, Nome varchar(30), Idade INT, Peso1 INT, Peso2 varchar(20))');
                        }).then(() => {
        ...
        return new Promise((resolve) => {
            ...
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    ...
                    tx.executeSql('SELECT * FROM cadastroItem', []).then(([tx, results]) => {
                        console.log("Consulta completa");
                        var len = results.rows.length;
                        for (let i = 0; i < len; i++) {
                            let row = results.rows.item(i);
                            const { id, Nome, Idade, Peso1, Peso2 } = row;
                            lista.push({ id, Nome,Idade, Peso1, Peso2 });
                        }
                        console.log(lista);
                        resolve(lista);
                    });
    ...
    Inserir(cadastro) {  //**** CRUD => UPDATE - aqui um registro da tabela é inserido *****/
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    //Query SQL para inserir um novo registro 
                    tx.executeSql('INSERT INTO cadastroItem (Nome, Idade, Peso1, Peso2) VALUES (?, ?, ?, ?)', [cadastro.Nome, cadastro.Idade, cadastro.Peso1, cadastro.Peso2]).then(([tx, results]) => {
                        resolve(results);
                    });
                }).then((result) => {
                    this.Desconectar(db);
    ...
    Atualizar(cadastro) {   //**** CRUD => UPDATE - aqui a tabela é atualizada *****/
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    //Query SQL para atualizar um registro no banco        
                    tx.executeSql('UPDATE cadastroItem SET Peso1 = ? WHERE id = ?', [cadastro.Peso1 , cadastro.id]).then(([tx, results]) => {
                        resolve(results);
                    });
                }).then((result) => {
                    this.Desconectar(db);
        ...
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    ...
                    tx.executeSql('DELETE FROM cadastroItem WHERE id = ?', [id]).then(([tx, results]) => {
                        console.log(results);
                        resolve(results);
                    });
                }).then((result) => {
